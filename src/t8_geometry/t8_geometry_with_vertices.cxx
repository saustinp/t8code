...
/* Load the coordinates of the newly active tree into this thread's
 * cached TLSEntry. (Previously `active_tree_vertices` was an instance
 * member; now it lives in the base class's per-thread cache.) */
void
t8_geometry_with_vertices::t8_geom_load_tree_data (t8_cmesh_t cmesh, t8_gloidx_t gtreeid)
{
  ...
  /* Load this tree's vertices into the thread-local cache. */
  const t8_locidx_t ltreeid = t8_cmesh_get_local_id (cmesh, gtreeid);
  const double *vertices = t8_cmesh_get_tree_vertices (cmesh, ltreeid);
  set_active_tree_vertices (vertices);
#if T8_ENABLE_DEBUG
  SC_CHECK_ABORTF (vertices != NULL, "ERROR: No vertices found for tree %li\n", (long) ltreeid);
#endif
}
...
{
  /* Cache thread-local lookups once at function entry; both for clarity
   * and to amortize the TLSEntry lookup cost. */
  const t8_eclass_t this_tree_class = active_tree_class ();
  const double *vertices = active_tree_vertices ();

  if (t8_eclass_to_dimension[this_tree_class] <= 2) {
    /* Only three dimensional eclass do have a volume */
    return false;
  }
  T8_ASSERT (t8_eclass_to_dimension[this_tree_class]
             == 3);  // Should we include 4 dimensional classes, we need to catch that here and implement it.
  T8_ASSERT (this_tree_class == T8_ECLASS_TET || this_tree_class == T8_ECLASS_HEX
             || this_tree_class == T8_ECLASS_PRISM || this_tree_class == T8_ECLASS_PYRAMID);

  T8_ASSERT (t8_eclass_num_vertices[this_tree_class] >= 4);
  /*
  ...
  int jvector_source;
  if (this_tree_class == T8_ECLASS_TET || this_tree_class == T8_ECLASS_PRISM) {
  ...
  for (int dim = 0; dim < T8_ECLASS_MAX_DIM; dim++) {
    v_1[dim] = vertices[3 + dim] - vertices[dim];
    v_2[dim] = vertices[6 + dim] - vertices[dim];
    v_j[dim] = vertices[3 * jvector_source + dim] - vertices[dim];
  }
  ...
  T8_ASSERT (sc_prod != 0);
  return this_tree_class == T8_ECLASS_TET ? sc_prod > 0 : sc_prod < 0;
}
